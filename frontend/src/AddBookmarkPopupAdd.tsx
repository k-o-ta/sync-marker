...
const AddBookmarkPopupAdd: React.FC = () => {
  ...
  const [book, _] = useState({
    isbn: data && data.bookFromIsbn.isbn.code,
    pageCount: data && data.bookFromIsbn.page,
    title: data && data.bookFromIsbn.name,
    datasource: data && data.bookFromIsbn.dataSource
  });
  const [
    createBook,
    { error: createBookError, data: createBookData }
  ] = useMutation<TCreateBookQuery>(CreateBookQuery);
