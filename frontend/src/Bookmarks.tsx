...
import {ProgressPopupContext, ProgressPopupStatus} from "./progressPopup/progressPopupContext";
...
const Bookmarks: React.FC = () => {
  ...
  function handleClick(e: React.MouseEvent<HTMLLIElement, MouseEvent>) {
    ...
    progressPopupContext.setStatus(ProgressPopupStatus.add, e.currentTarget.dataset.bookmarkIsbn, parseInt(e.currentTarget.dataset.pageCount||"0"))
        ...
        <ProgressPopupContext.Provider value={{status: progressState, setStatus: (popup: ProgressPopupStatus, isbn?: string, pageCount?: number) => {console.log("set");setProgressState({popup, isbn, pageCount})}}}>
          <ul>
            {data.bookmarks.map(bookmark => (
              ...
              >
                {bookmark.title}: {bookmark.pageInProgress}/{bookmark.pageCount}ページ
              </li>
            ))}
          </ul>
          {console.log("hoge")}
          {console.log(progressState.popup)}
          {(progressState.popup === ProgressPopupStatus.add) && <ProgressPopupAdd/>}
