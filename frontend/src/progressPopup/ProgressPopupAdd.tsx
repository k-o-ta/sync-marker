...
import {ProgressPopupContext, ProgressPopupStatus} from "./progressPopupContext";
// import {CreateBookQuery} from "./queries/bookQuery";
const ProgressPopupAdd: React.FC = () => {
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    await progress({variables: { isbn: progressPopupContext.status.isbn || "", pageCount: pageCount}});
    progressPopupContext.setStatus(ProgressPopupStatus.hide, progressPopupContext.status.isbn, pageCount);
  }
  const progressPopupContext = useContext(ProgressPopupContext);
  const [pageCount, setPageCount] = useState(progressPopupContext.status.pageCount)
